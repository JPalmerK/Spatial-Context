function [GPL_struct] = GPL_v3(data, parm); 
...
if isempty(sp)
    GPL_struct = [];
    disp('GPL_fft:exit on badly formed signal stream')
    return;
end
...
if all(all(isnan(sp_whiten)))     
    GPL_struct = [];
    disp('GPL_whiten:exit on badly formed signal stream')    
    return;
end
