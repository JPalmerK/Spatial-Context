...
for k=1:length(start)
    ...
    if(isfield(GPL_struct,'cm'));
        cm = GPL_full('cm',k,GPL_struct); end
    if(isfield(GPL_struct,'cm_max')); 
        cm_max =  GPL_full('cm_max',k,GPL_struct); end
    if(isfield(GPL_struct,'cm_max2')); 
        cm_max2 = GPL_full('cm_max2',k,GPL_struct); end
    
    
    
    
    if(parm.measurementsments.slope == 1 || parm.measurements.cm_max_duration ==1)
    ...
    if(parm.measurements.slope == 1 || parm.measurements.cm_max2_duration ==1)
    ...
    if(parm.measurements.cm_max_bandwidth == 1)
    ...
    if(parm.measurements.cm_max2_bandwidth == 1)
    ...
    if(parm.measurements.spec_noise==1)
        
        GPL_struct(k).spec_noise = quiet_fft;
    end
    
    if(parm.measurements.spec_rl==1)
