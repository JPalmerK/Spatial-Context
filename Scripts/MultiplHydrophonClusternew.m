...
function [chain, Cluster_id] = MultiplHydrophonClusternew(array_struct,...
    TDOAs, array_idx,fs, NChidlHyd, time_cut,...
...
% Create the TDOA space
Px = zeros(NChidlHyd,...
    size(localize_struct.hyd(ParentHyd).delays,1),...
    length(array_struct(1).latgrid) *...
    length(array_struct(1).longrid), 1);


%Create the correlation plots for each parent/child pair
Corr_coef_map = zeros(NChidlHyd,...
    length(TDOAs(ParentHyd,:)),...
    length(TDOAs(ParentHyd,:)));

% set sigma equal to 1 for now
...
for ii=1:NChidlHyd
    ...
    for kk = 1:length(TDOAs(ParentHyd,:))
...
% Need to allow sigma to growwith time, create sigma matrix that documents
% sigma between each call
...
for ii =1:NChidlHyd
    for kk = 1:size(length(TDOAs(ParentHyd,:)),1)
        ...
        arrival_times = rtimes/fs + (delays(ii));
        
        % Difference in arrival times in seconds
        diff_times = arrival_times-arrival_times(kk);
        sigma = (diff_times * (2*s)/c);
        
        % Create the growing map and cross correlate it with the remaining
        % calls in the system
        for mm = kk:size(localize_struct.hyd(ParentHyd).delays,1)
            
            % Grow the LSQ space
            LSQ_space =  normpdf(LSQ_VAL, 0, abs(1+sigma(mm)));            
            ...
            coef = length(intersect(bb_call, aa_call))/size_bb;
            
            
            %coef = corrcoef(reshape(LSQ_space.',[] ,1), (Px(ii,mm,:)));
            % Correlate it was the LSQ space for the next call
            Corr_coef_map(ii,kk,mm)=  coef;
            Corr_coef_map(ii,mm,kk)=  coef;
                   
        end
       
    disp([num2str(kk) ' of ' num2str(size(localize_struct.hyd(ParentHyd).delays,1))])
    end

    imagesc(squeeze(Corr_coef_map(ii,:,:)), [0 1])
    xlabel('Call Number')
    ylabel('Call Number')
    title(['Hydrophones ' num2str(ParentHyd) ' and ' num2str(ChildHyds(ii))])
...
if NChidlHyd>1
    Corr_coef_map_out = squeeze(min(Corr_coef_map));
else
...
Corr_coef_map_out(isnan(Corr_coef_map_out)) = 1;
...
if isfield(localize_struct.hyd(ParentHyd), 'agent_ids')
    ...
    ncolors = max([max(Cluster_id)+1 length(unique(localize_struct.hyd(ParentHyd).agent_ids))]);
    ColorVals = lines(ncolors);
    
    
    % Color scales for simulated clusters
    color_var_drift = ColorVals(Cluster_id+1,:);
    ...
    title1 =[ 'Localized Data Clusterd Using Ladder Structure and Mean' newline 'Correlation Between ', num2str(NChidlHyd),' Hydrophone Pairs'];
    title2 = 'Localized Data Plotted Against Time of Detection';
    
    color_var_drift= zeros(size(x));
    ColorVals = jet(length(k2));
    colorscale2 = (localize_struct.hyd(ParentHyd).rtimes(k2)- min(localize_struct.hyd(ParentHyd).rtimes(k2)))/fs;
...
if ~isempty(hydrophone_struct)
    ...
    scatter(hyd_arr(array_struct.master,2), hyd_arr(array_struct.master,1), 100, 'r', 'filled', '^');
    scatter(hyd_arr(1:NChidlHyd,2), hyd_arr(1:NChidlHyd,1), 100, 'r', 'filled', '^');
