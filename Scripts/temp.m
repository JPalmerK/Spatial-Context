...
for ii=1:7
    ...
    parm_movement.duration = 60*60*(n_hrs); % how long is the agent around
    
    
    %parm_movement.start_time = randi([0, param_sim.dur-parm_movement.duration],1)
    parm_movement.start_time = 1;
...
scatter(hyd_arr(1,2), hyd_arr(1,1), 80, 'k', 'filled', 's')
...
time_cut = 8*60; % don't correlate anything greater than 5 min apart
for jj = 2:2
    for ii = 1:1
    ...
    [chain1, Cluster_id1 ] = MultiplHydrophonClusterDelays(array_struct,...
    localize_struct_sim,array_idx,fs, NChidlHyd, time_cut,...
    cor_thresh, [], hydrophone_struct)
